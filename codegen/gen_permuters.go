...
func writePermuteFunction(f *os.File, id int, order [11]int) {
	...
	fmt.Fprintf(f, "func (a *permuter) adaptCustom%d(val int) {\n", id)
	terms := []string{}
	...
	fmt.Fprintf(f, "\t\ta.out.Transmit(val)\n")
	fmt.Fprintf(f, "\t}\n")
	fmt.Fprintf(f, "}\n\n")
}
...
func main() {
	...
	fmt.Fprintf(f, "func getCustomPermuter(order [11]int) (func(*permuter, int), bool) {\n")
	for i, order := range orders {
		fmt.Fprintf(f, "\tif order == %#v {\n", order)
		fmt.Fprintf(f, "\t\treturn (*permuter).adaptCustom%d, true\n", i)
		fmt.Fprintf(f, "\t}\n")
	}
